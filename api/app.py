...
def _generate_scripts(wid: str, gemini_key: str, channel_name: str, count: int, sound_lines: List[str]) -> List[str]:
    ...
    prompt = f"""
KAMU ADALAH PENULIS NASKAH SHORT/REELS.

INI ADALAH KUMPULAN CONTOH KALIMAT ACUAN (SUMBER):
{reference_text}

TUGAS:
BUAT {count} VARIASI NASKAH BARU YANG SANGAT MIRIP POLA BAHASANYA DENGAN SUMBER DI ATAS,
DENGAN ATURAN WAJIB:

1) OUTPUT HANYA LIST, TANPA PENJELASAN. 1 BARIS = 1 NASKAH.
2) SEMUA HURUF HARUS KAPITAL.
3) TIAP NASKAH MAKSIMAL 1–2 KALIMAT PENDEK (GAYA < 15 DETIK).
4) WAJIB ADA CTA: LIKE, SUBSCRIBE, SALIN TAUTAN, DAN KOMEN (SEMUA MUNCUL).
5) WAJIB MENAWARKAN “AKUN SULTAN GRATIS” ATAU “AKUN SULTAN” (SALA SATU).
6) JANGAN MEMBUAT TOPIK BARU DI LUAR TEMA “AKUN SULTAN GRATIS / AKUN BOT / ELITE PASS / SKIN BANYAK”.
7) PAKAI KATA/FRASA YANG SUDAH ADA DI SUMBER (BOLEH DIROMBAK URUTANNYA), TAPI JANGAN COPY PASTE UTUH.
8) SETIAP ADA NAMA “BANG …” DI SUMBER, GANTI MENJADI: "{channel_name}".
   - JIKA "{channel_name}" TIDAK DIAWALI KATA "BANG", MAKA TAMBAHKAN KATA "BANG" DI DEPANNYA SAAT DIPAKAI.
9) TIAP BARIS HARUS BERBEDA (JANGAN DUPLIKAT).

FORMAT OUTPUT CONTOH (HANYA CONTOH FORMAT):
- AKUN SULTAN GRATIS INI BUAT KAMU, MAU? LIKE, SUBSCRIBE, SALIN TAUTAN, DAN KOMEN MAU BANG XYZ
- WOI AKUN KAMU MASIH BOT? BANG XYZ BAGI AKUN SULTAN GRATIS, LIKE, SUBSCRIBE, SALIN TAUTAN, KOMEN MAU!

SEKARANG TULIS LIST NASKAHNYA.
""".strip()
